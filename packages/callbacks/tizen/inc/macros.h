...
#if __WORDSIZE == 64
#define INVALID_USER_DATA(user_data) \
  !user_data || reinterpret_cast<uintptr_t>(user_data) >> 32
#define GET_CALLBACK_ID_FROM_USER_DATA(user_data) \
  (uint32_t) reinterpret_cast<uintptr_t>(user_data)
#else
#define INVALID_USER_DATA(user_data) !user_data
#define GET_CALLBACK_ID_FROM_USER_DATA(user_data) \
  reinterpret_cast<uint32_t>(user_data)
#endif
...
#define PROXY_GROUP_RETURN(CB_NAME, CB_RETURN, CB_PARAMS...)                 \
  CB_RETURN platform_blocking_##CB_NAME(CB_PARAMS) {                         \
    if (INVALID_USER_DATA(user_data)) {                                      \
      LOG_ERROR("Received bad user_data in %s: %p - ignoring", #CB_NAME,     \
                user_data);                                                  \
      return CB_RETURN();                                                    \
    }                                                                        \
    uint32_t _callback_id = GET_CALLBACK_ID_FROM_USER_DATA(user_data);       \
    ...
    CallbackInfo _callback_info = _cb_id_to_info_it->second;                 \
    if (gettid() == interop_callbacks_thread_id) {                           \
      LDEBUG("calling dart callback directly");                              \
      return (reinterpret_cast<CB_NAME>(                                     \
          _callback_info.callback_ptr))(EXPAND_LIST(CB_PARAMS_NAMES));       \
    }                                                                        \
    ...
    bool _callback_finished = false;                                         \
                                                                             \
    CallbackWrapper wrapper = [_callback_info, EXPAND_LIST(CB_PARAMS_NAMES), \
                               &_cv, &_callback_finished, &_return_value] {  \
      CB_NAME local_cb_ptr =                                                 \
          reinterpret_cast<CB_NAME>(_callback_info.callback_ptr);            \
      LDEBUG("calling local_cb_ptr() %p, user_data:%p", local_cb_ptr,        \
             user_data);                                                     \
      if (local_cb_ptr)                                                      \
        _return_value = local_cb_ptr(EXPAND_LIST(CB_PARAMS_NAMES));          \
...
#define PROXY_GROUP_BLOCKING(CB_NAME, CB_PARAMS...)                          \
  void platform_blocking_##CB_NAME(CB_PARAMS) {                              \
    ...
    bool _callback_finished = false;                                         \
                                                                             \
    CallbackWrapper wrapper = [_callback_info, EXPAND_LIST(CB_PARAMS_NAMES), \
                               &_cv, &_callback_finished] {                  \
      CB_NAME local_cb_ptr =                                                 \
          reinterpret_cast<CB_NAME>(_callback_info.callback_ptr);            \
      LDEBUG("calling local_cb_ptr() %p, user_data:%p", local_cb_ptr,        \
             user_data);                                                     \
      if (local_cb_ptr) local_cb_ptr(EXPAND_LIST(CB_PARAMS_NAMES));          \
      _callback_finished = true;                                             \
      LDEBUG("calling notify_one()");                                        \
      _cv.notify_one();                                                      \
    };                                                                       \
...
#define PROXY_GROUP_BLOCKING_NO_USER_DATA(CB_NAME, CB_PARAMS...)             \
  void platform_blocking_##CB_NAME(uint32_t _callback_id, CB_PARAMS) {       \
    ...
    LDEBUG("exit");                                                          \
                                                                             ...
                                                                             \
  void platform_blocking_##CB_NAME##_0(CB_PARAMS) {                          \
    platform_blocking_##CB_NAME(GET_CALLBACK_ID(CB_NAME, 0),                 \
                                EXPAND_LIST(CB_PARAMS_NAMES));               \
  }                                                                          \
  void platform_blocking_##CB_NAME##_1(CB_PARAMS) {                          \
    platform_blocking_##CB_NAME(GET_CALLBACK_ID(CB_NAME, 1),                 \
                                EXPAND_LIST(CB_PARAMS_NAMES));               \
  }                                                                          \
  void platform_blocking_##CB_NAME##_2(CB_PARAMS) {                          \
    platform_blocking_##CB_NAME(GET_CALLBACK_ID(CB_NAME, 2),                 \
                                EXPAND_LIST(CB_PARAMS_NAMES));               \
  }                                                                          \
  void platform_blocking_##CB_NAME##_3(CB_PARAMS) {                          \
    platform_blocking_##CB_NAME(GET_CALLBACK_ID(CB_NAME, 3),                 \
                                EXPAND_LIST(CB_PARAMS_NAMES));               \
  }                                                                          \
  void platform_blocking_##CB_NAME##_4(CB_PARAMS) {                          \
    platform_blocking_##CB_NAME(GET_CALLBACK_ID(CB_NAME, 4),                 \
                                EXPAND_LIST(CB_PARAMS_NAMES));               \
  }
...
#define PROXY_GROUP_RETURN_NO_USER_DATA_NO_PARAM(CB_NAME, CB_RETURN)          \
  CB_RETURN platform_blocking_##CB_NAME(uint32_t _callback_id) {              \
    ...
    if (gettid() == interop_callbacks_thread_id) {                            \
      CallbackInfo _callback_info = _cb_id_to_info_it->second;                \
      LDEBUG("calling dart callback directly");                               \
      return (reinterpret_cast<CB_NAME>(_callback_info.callback_ptr))();      \
    }                                                                         \
    CB_RETURN _return_value = CB_RETURN();                                    \
    std::mutex _mutex;                                                        \
    std::unique_lock<std::mutex> lock(_mutex);                                \
    ...
    CallbackInfo _callback_info = _cb_id_to_info_it->second;                  \
                                                                              \
    CallbackWrapper wrapper = [_callback_info, &_cv, &_callback_finished,     \
                               &_return_value] {                              \
      CB_NAME local_cb_ptr =                                                  \
          reinterpret_cast<CB_NAME>(_callback_info.callback_ptr);             \
      LDEBUG("calling local_cb_ptr() %p", local_cb_ptr);                      \
