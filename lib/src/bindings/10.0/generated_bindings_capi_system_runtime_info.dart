/// {@category 10.0/tizen}
library tizen_interop_10_0.capi_system_runtime_info;
...
///
/// This structure provides comprehensive memory usage information collected from /proc/meminfo. The memory values are reported in kibibytes (KiB) where 1 KiB = 1024 bytes. The memory metrics:
/// - `total`: Total physical memory (MemTotal field from /proc/meminfo)
/// - `used`: Actually used memory, calculated as (total - available) when MemAvailable is available, otherwise (total - free - cache) when MemAvailable is not available
/// - `free`: Completely unused memory that can be allocated immediately (MemFree field)
/// - `cache`: Memory used for file system cache and buffers (Cached field)
/// - `swap`: Used swap space on disk, calculated as (SwapTotal - SwapFree)
///
/// **Since Tizen:**
/// - 2.4
/// @nodoc
...
///
/// This structure provides detailed memory usage information for specific processes. The information is collected from /proc/`[pid]`/stat and /proc/`[pid]`/smaps files. All values are reported in kibibytes (KiB). Memory metrics:
...
///
/// This structure provides CPU usage distribution across different states. The values are percentages calculated since system boot and represent the time CPU spent in each state. The information is collected from /proc/stat. Each field is calculated as a percentage using the formula: percentage = (field_value / total_uptime) × 100 where total_uptime = user + nice + system + idle + iowait + irq + softirq (values collected from /proc/stat cpu line fields 1-7 in clock ticks) CPU usage metrics:
/// - `user`: Percentage of time spent running user-mode processes (applications), calculated as (user_value / total_uptime) × 100, where user_value is collected from /proc/stat field 1 (user mode time in clock ticks)
/// - `system`: Percentage of time spent running kernel-mode processes (system calls, drivers), calculated as (system_value / total_uptime) × 100, where system_value is collected from /proc/stat field 3 (system mode time in clock ticks)
/// - `nice`: Percentage of time spent running user-mode processes with lowered priority, calculated as (nice_value / total_uptime) × 100, where nice_value is collected from /proc/stat field 2 (nice priority user mode time in clock ticks)
/// - `iowait`: Percentage of time spent waiting for I/O operations to complete (disk, network), calculated as (iowait_value / total_uptime) × 100, where iowait_value is collected from /proc/stat field 5 (I/O wait time in clock ticks)
///
/// **Since Tizen:**
/// - 2.4
/// @nodoc
...
///
/// This structure provides CPU usage information for specific processes. The information is collected from /proc/`[pid]`/stat file. All values are reported in clock ticks. CPU usage metrics:
/// - `utime`: Amount of time that this process has been scheduled in user mode, collected from /proc/`[pid]`/stat field 14 (user mode time in clock ticks)
/// - `stime`: Amount of time that this process has been scheduled in kernel mode, collected from /proc/`[pid]`/stat field 15 (kernel mode time in clock ticks)
///
/// **Since Tizen:**
/// - 2.4
/// @nodoc
