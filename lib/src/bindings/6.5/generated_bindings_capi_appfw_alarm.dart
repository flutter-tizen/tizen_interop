/// {@category 6.5/tizen}
library tizen_interop_6_5.capi_appfw_alarm;
...
/// {@category 6.5/tizen}
class Tizen65CapiAppfwAlarm {
  /// Holds the symbol lookup function.
  final ffi.Pointer<T> Function<T extends ffi.NativeType>(String symbolName)
      _lookup;

  /// The symbols are looked up in [dynamicLibrary].
  Tizen65CapiAppfwAlarm(ffi.DynamicLibrary dynamicLibrary)
      : _lookup = dynamicLibrary.lookup;

  /// The symbols are looked up with [lookup].
  Tizen65CapiAppfwAlarm.fromLookup(
  ...
  /// - Since @if MOBILE 2.4 @elseif WEARABLE 3.0 @endif If `app_control` is specified with service-application, the application is only allowed to run on which has Background Category.
  /// - Since @if MOBILE 2.4 @elseif WEARABLE 3.0 @endif If the appid of `app_control` is not specified, this function is not allowed. In other words, the explicit `app_control` is only allowed.
  ...
  /// - `delay` (in): The amount of time before the first execution (in seconds). Since @if MOBILE 2.4 @elseif WEARABLE 3.0 @endif, Although this is inexact, the alarm will not fire before this time
  ...
  /// **Deprecated:** Deprecated since Mobile 2.4; Wearable 3.0. `[Use alarm_schedule_once_at_date() instead]`
  ///
  ...
  /// - If the operation of `app_control` is not specified, `APP_CONTROL_OPERATION_DEFAULT` is used for the launch request.
  /// - If the operation of `app_control` is `APP_CONTROL_OPERATION_DEFAULT`, the package information is mandatory to explicitly launch the application.
  /// - Since @if MOBILE 2.4 @elseif WEARABLE 3.0 @endif, The `app_control` only supports UI application with this function. If `app_control` is not UI application, `ALARM_ERROR_NOT_PERMITTED_APP` returned.
  /// - When the alarm is expired, Alarm Manager will turn on LCD to prohibit background jobs.
  /// - Since @if MOBILE 2.4 @elseif WEARABLE 3.0 @endif, If the appid of `app_control` is not specified, this function is not allowed. In other words, the explicit `app_control` is only allowed.
  /// - Since 4.0, http://tizen.org/privilege/appmanager.launch privilege is additionally required.
  ///
  ...
  /// **Parameters:**
  /// - `alarm_id` (in): The ID which uniquely identifies the scheduled alarm
  /// - `period` (in): The amount of time between subsequent alarms (in seconds). Since @if MOBILE 2.4 @elseif WEARABLE 3.0 @endif, this value does not guarantee the accuracy. The actual interval is calculated by the OS. The minimum value is 600sec.
  ///
  /// **Returns:**
  /// - `0` on success, otherwise a negative error value
  ///
